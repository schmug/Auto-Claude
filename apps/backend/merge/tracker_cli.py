...
Usage:
    python -m auto_sleuth.merge.tracker_cli notify-commit <hash>
    python -m auto_sleuth.merge.tracker_cli show-timeline <file_path>
    python -m auto_sleuth.merge.tracker_cli show-drift <task_id>
"""
...
def find_project_root() -> Path:
    """Find the project root by looking for .auto-sleuth or .git directory."""
    current = Path.cwd()

    # Walk up until we find .auto-sleuth or .git
    while current != current.parent:
        if (current / ".auto-sleuth").exists() or (current / ".git").exists():
            return current

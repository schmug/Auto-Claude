...
def run_discovery_script(
    ...
    """
    case_index = case_dir / "project_index.json"
    auto_build_index = project_dir / "auto-sleuth" / "project_index.json"

    # Check if project_index already exists
    if auto_build_index.exists() and not case_index.exists():
        # Copy existing index
        shutil.copy(auto_build_index, case_index)
    ...
    cmd = [sys.executable, str(script_path), "--output", str(case_index)]
    ...
    try:
        ...
        if result.returncode == 0 and case_index.exists():
...
def get_project_index_stats(case_dir: Path) -> dict:

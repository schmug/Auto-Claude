...
class DiscoveryPhaseMixin:
    ...
    async def phase_discovery(self) -> PhaseResult:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if success:
                stats = discovery.get_project_index_stats(self.case_dir)
                if stats:
                ...
                case_index = self.case_dir / "project_index.json"
                return PhaseResult("discovery", True, [str(case_index)], [], retries)
    ...
    async def phase_context(self) -> PhaseResult:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if success:
                stats = context.get_context_stats(self.case_dir)
                if stats:
                    self.task_logger.log(
        ...
        context.create_minimal_context(self.case_dir, task or "unknown task", services)

...
class ComplexityAssessment:
    ...
    def phases_to_run(self) -> list[str]:
        ...
        if self.complexity == Complexity.SIMPLE:
            return ["discovery", "historical_context", "quick_case", "evidence_preprocessing", "validation"]
            ...
            phases.extend(["context", "case_writing", "evidence_preprocessing", "planning", "validation"])
            return phases
        else:  # COMPLEX
            return [
                "discovery",
...
async def run_ai_complexity_assessment(
    ...
    context = f"""
**Project Directory**: {case_dir.parent.parent}
...
"""
    ...
    auto_build_index = case_dir.parent.parent / "auto-sleuth" / "project_index.json"
...
def save_assessment(case_dir: Path, assessment: ComplexityAssessment) -> Path:

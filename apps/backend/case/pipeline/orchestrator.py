...
from ..validate_pkg.case_validator import CaseValidator
...
from .models import (
    ...
    create_case_dir,
    get_cases_dir,
    rename_case_dir_from_requirements,
)


class CaseOrchestrator:
    """Orchestrates the case creation process with dynamic complexity adaptation."""
    ...
    def __init__(
        ...
        case_dir: Path
        | None = None,  # Use existing case directory (for UI integration)
        model: str = "sonnet",  # Shorthand - resolved via API Profile if configured
        thinking_level: str = "medium",  # Thinking level for extended thinking
        complexity_override: str | None = None,  # Force a caseific complexity
        use_ai_assessment: bool = True,  # Use AI for complexity assessment (vs heuristics)
    ):
        """Initialize the case orchestrator.

        Args:
            project_dir: The project root directory
            task_description: Optional task description
            case_name: Optional case name (for existing cases)
            case_dir: Optional existing case directory (for UI integration)
            model: The model to use for agent execution
            thinking_level: Thinking level (none, low, medium, high, ultrathink)
            complexity_override: Force a caseific complexity level
        ...
        # Get the appropriate cases directory (within the project)
        self.cases_dir = get_cases_dir(self.project_dir)

        # Clean up orphaned pending folders before creating new case
        cleanup_orphaned_pending_folders(self.cases_dir)
        ...
        if case_dir:
            # Use provided case directory (from UI)
            ...
            # Use lock for coordinated case numbering across worktrees
            with CaseNumberLock(self.project_dir) as lock:
                self.case_dir = create_case_dir(self.cases_dir, lock)
                # Create directory inside lock to ensure atomicity
                self.case_dir.mkdir(parents=True, exist_ok=True)
        self.validator = CaseValidator(self.case_dir)
    ...
    def _get_agent_runner(self) -> AgentRunner:
        ...
        if self._agent_runner is None:
            task_logger = get_task_logger(self.case_dir)
            self._agent_runner = AgentRunner(
                self.project_dir, self.case_dir, self.model, task_logger
            )
    ...
    async def _run_agent(
        ...
        # Use user's configured thinking level for all case phases
    ...
    async def _store_phase_summary(self, phase_name: str) -> None:
        ...
        try:
            ...
            phase_output = gather_phase_outputs(self.case_dir, phase_name)
            if not phase_output:
                return
    ...
    async def _ensure_fresh_project_index(self) -> None:
        """Ensure project_index.json is up-to-date before case creation.
        ...
        if should_refresh_project_index(self.project_dir):
            ...
            try:
                ...
                # Don't fail case creation if indexing fails - continue with cached/missing
        else:
            if index_file.exists():
    ...
    async def run(self, interactive: bool = True, auto_approve: bool = False) -> bool:
        """Run the case creation process with dynamic phase selection.
        ...
        Returns:
            True if case creation and review completed successfully, False otherwise
        """
        ...
        task_logger = get_task_logger(self.case_dir)
        task_logger.start_phase(LogPhase.PLANNING, "Starting case creation process")
        ...
        # Rename case folder with better name from requirements
        rename_case_dir_from_requirements(self.case_dir)
        ...
        all_phases = {
            ...
            "case_writing": phase_executor.phase_case_writing,
            "self_critique": phase_executor.phase_self_critique,
            "evidence_preprocessing": phase_executor.phase_evidence_preprocessing,  # DFIR: runs before planning
            "planning": phase_executor.phase_planning,
            "validation": phase_executor.phase_validation,
            "quick_case": phase_executor.phase_quick_case,
        }
        ...
        for phase_name in phases_to_run:
            ...
            if not result.success:
                ...
                print()
                print_status(
                    "Case creation incomplete. Fix errors and retry.", "warning"
                )
                task_logger.log(
        ...
        task_logger.end_phase(
            LogPhase.PLANNING, success=True, message="Case creation complete"
        )
    ...
    async def _create_linear_task_if_enabled(self) -> None:
        ...
        linear_state = await create_linear_task(
            case_dir=self.case_dir,
            title=self.task_description or self.case_dir.name,
            description=f"Auto-build case: {self.case_dir.name}",
        )
        if linear_state:
    ...
    async def _phase_complexity_assessment_with_requirements(
        ...
        if not assessment_file.exists():
            complexity.save_assessment(self.case_dir, self.assessment)
...
"""
    ...
    def _heuristic_assessment(self) -> complexity.ComplexityAssessment:
        ...
        """
        project_index = {}
        auto_build_index = self.project_dir / "auto-sleuth" / "project_index.json"
    ...
    def _generate_case_name(self, task_description: str) -> str:
        """Generate a case name from task description (backward compatibility).

        This method is kept for backward compatibility with existing tests.
        The functionality has been moved to models.generate_case_name.
        ...
        Returns:
            Generated case name
        """
        from .models import generate_case_name

        return generate_case_name(task_description)

    def _rename_case_dir_from_requirements(self) -> bool:
        """Rename case directory from requirements (backward compatibility).

        This method is kept for backward compatibility with existing tests.
        The functionality has been moved to models.rename_case_dir_from_requirements.

        Returns:
            True if successful or not needed, False on error
        """
        result = rename_case_dir_from_requirements(self.case_dir)
        # Update self.case_dir if it was renamed
        if result and self.case_dir.name.endswith("-pending"):
            # Find the renamed directory
            parent = self.case_dir.parent
            prefix = self.case_dir.name[:4]  # e.g., "001-"

...
from ..schemas import INVESTIGATION_PLAN_SCHEMA


class InvestigationPlanValidator:
    """Validates investigation_plan.json exists and has valid schema."""
    ...
    def validate(self) -> ValidationResult:
        ...
        if not plan_file.exists():
            errors.append("investigation_plan.json not found")
            fixes.append(
                f"Run: python auto-sleuth/planner.py --case-dir {self.case_dir}"
            )
        ...
        try:
            ...
            errors.append(f"investigation_plan.json is invalid JSON: {e}")
            fixes.append(
                "Regenerate with: python auto-sleuth/planner.py --case-dir "
                + str(self.case_dir)
            )
            return ValidationResult(False, "plan", errors, warnings, fixes)
    ...
    def _validate_phase(self, phase: dict, index: int) -> list[str]:
        ...
        """
        errors = []
        schema = INVESTIGATION_PLAN_SCHEMA["phase_schema"]

...
Usage:
    ...
    strategy = builder.build_strategy(project_dir, case_dir, "medium")
...
def detect_project_type(project_dir: Path) -> str:
    ...
    # Check for caseific frameworks first
...
class ValidationStrategyBuilder:
    ...
    def build_strategy(
        ...
        """
        Build a validation strategy for the given project and case.
        ...
        if risk_level is None:
            assessment = self._risk_classifier.load_assessment(case_dir)
            if assessment:
    ...
    def _strategy_for_ruby(
        ...
        return ValidationStrategy(
            ...
            steps=steps,
            test_types_required=["unit"],
            reasoning="Ruby project requires RCase tests.",
        )
    ...
    def _add_security_steps(
        ...
        security_steps.append(
            ValidationStep(
                name="Secrets Scan",
                command="python auto-sleuth/scan_secrets.py --all-files --json",
...
def build_validation_strategy(
    ...
    """
    builder = ValidationStrategyBuilder()
    return builder.build_strategy(project_dir, case_dir, risk_level)
...
def get_strategy_as_dict(
    ...
    """
    builder = ValidationStrategyBuilder()
    strategy = builder.build_strategy(project_dir, case_dir, risk_level)
...
def main() -> None:
    ...
    case_dir = args.case_dir or args.project_dir
    builder = ValidationStrategyBuilder()
    strategy = builder.build_strategy(args.project_dir, case_dir, args.risk_level)

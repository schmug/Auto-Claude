      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        ...
        try {
          ...
          if (options?.forceApprove) {
            ...
            body = `## ✅ Auto Sleuth Review - APPROVED\n\n`;
            ...
            body += `*Generated by Auto Sleuth*`;
          } else {
            // Standard review format
            body = `## 🤖 Auto Sleuth PR Review\n\n${result.summary}\n\n`;
            ...
            body += `---\n*This review was generated by Auto Sleuth.*`;
          }
    ...
    async (_, projectId: string, prNumber: number, body: string): Promise<boolean> => {
      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        try {
          ...
          const tmpFile = join(project.path, project.autoBuildPath || '.auto-sleuth', 'tmp_comment_body.txt');
          try {

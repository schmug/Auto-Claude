...
export function registerProjectHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<Project[]>> => {
      // Validate that .auto-sleuth folders still exist for all projects
      ...
      if (resetIds.length > 0) {
        console.warn('[IPC] PROJECT_LIST: Detected missing .auto-sleuth folders for', resetIds.length, 'project(s)');
      }
    ...
    async (_, projectId: string): Promise<IPCResult<InitializationResult>> => {
      try {
        ...
        if (result.success) {
          ...
          projectStore.updateAutoBuildPath(projectId, '.auto-sleuth');
        }
  ...
  // Version tracking for .auto-sleuth is removed since it only contains data
  ipcMain.handle(
      ...
      try {
        ...
        return {
          ...
          data: {
            ...
            updateAvailable: false // No updates for .auto-sleuth - it's just data
          }
        };
      } catch (error) {
        return {
  ...
  // Check if project has local auto-sleuth source (is dev project)
  ipcMain.handle(

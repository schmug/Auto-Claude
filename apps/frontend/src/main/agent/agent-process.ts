...
export class AgentProcessManager {
  ...
  getAutoBuildSourcePath(): string | null {
    // Use runner presence as the validation marker (case_runner or legacy spec_runner).
    const validatePath = (p: string): boolean => {
      ...
      return existsSync(p) && (existsSync(caseRunner) || existsSync(specRunner));
    };
  ...
  /**
   * Load environment variables from project's .auto-sleuth/.env file
   ...
   * 3. Project's .auto-sleuth/.env - Frontend-configured settings (memory, integrations)
